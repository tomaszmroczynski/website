  ...
  return (
    <form className={styles.form} method="post" action="/api/kontakt" onSubmit={onSubmit}>
      <div className={styles.field}>
        ...
        <input className={styles.input} id="cf-name" name="name" required maxLength={120} autoComplete="name" />
      </div>

      <div className={styles.field}>
        <label className={styles.label} htmlFor="cf-email">{t("email")}</label>
        <input className={styles.input} id="cf-email" name="email" type="email" required maxLength={200} autoComplete="email" />
      </div>

      <div className={styles.field}>
        <label className={styles.label} htmlFor="cf-message">{t("message")}</label>
        <textarea className={styles.textarea} id="cf-message" name="message" required minLength={10} maxLength={5000} />
      </div>
